import { inferMediaType, mediaTypeForNoteType, type MediaAssetDetail, type NoteDetail, type NoteType } from "@notextra/api";
...
export async function uploadMediaFromUri(input: {
	...
	const session = await api.media.initiateUpload({
		...
		contentType,
		type: mediaTypeForNoteType(input.noteType ?? "TEXT") ?? inferMediaType(contentType, input.fileName),
	});
